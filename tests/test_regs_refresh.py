...
def test_refresh_overrides_open_status_in_existing_data():
    existing = {
        ...
        "launches": [{"key": "vernita", "regs_section": "WDFW_HANFORD_REACH",
                      "parent_key": None, "species": ["chinook"]}],
        "top_picks": {"chinook": [{"launch": "vernita", "day_offset": 0, "score": 0.7, "technique": "x"}]},
    }
    ...
    assert updated["forecasts"]["chinook::vernita"][0]["score"] == 0.0
    # Top picks should be re-filtered to drop the now-closed pick
    assert updated["top_picks"]["chinook"] == []
