...
def test_parses_chinook_at_bonneville():
    ...
    bon_chin = [
        r for r in records
        if r.dam_key == "BON" and r.species in {"spring_chinook", "summer_chinook", "fall_chinook"}
    ]
...
def test_parses_steelhead_at_lower_granite():
    html = FIXTURE.read_text()
    records = parse_adult_counts(html)
    lgr_sth = [r for r in records if r.dam_key == "LGR" and r.species == "summer_steelhead"]
