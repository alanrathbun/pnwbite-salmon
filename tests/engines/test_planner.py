...
def test_top_launches_by_species_date_ranks_open_only(forecasts):
    out = top_launches_by_species_date(forecasts, "chinook", "2026-05-10", k=5)
...
def test_top_launches_caps_at_k(forecasts):
    out = top_launches_by_species_date(forecasts, "chinook", "2026-05-10", k=1)
    assert len(out) == 1
    assert out[0]["launch"] == "umatilla_marina"


def test_top_dates_by_launch_species_returns_best_dates(forecasts):
    out = top_dates_by_launch_species(forecasts, "mcnary_tailrace", "chinook", k=2)
...
def test_top_dates_excludes_closed_dates(forecasts):
    forecasts["chinook::mcnary_tailrace"][0]["open"] = False
    out = top_dates_by_launch_species(forecasts, "mcnary_tailrace", "chinook", k=5)
...
def test_top_pairs_by_date_ranks_across_species(forecasts):
    ...
    # umatilla chinook (0.92) > mcnary chinook (0.84) > mcnary steelhead (0.20)
    assert [(p["launch"], p["species"], p["score"]) for p in out] == [
        ("umatilla_marina", "chinook", 0.92),
        ("mcnary_tailrace", "chinook", 0.84),
        ("mcnary_tailrace", "steelhead", 0.20),
    ]
...
def test_season_heatmap_for_species_returns_best_open_score_per_date(forecasts):
    out = season_heatmap_for_species(forecasts, "chinook")
...
def test_top_n_breaks_ties_alphabetically(forecasts):
    ...
    forecasts["chinook::aaa_launch"] = [_make_entry("2026-05-13", 0.50)]
    forecasts["chinook::bbb_launch"] = [_make_entry("2026-05-13", 0.50)]
    out = top_launches_by_species_date(forecasts, "chinook", "2026-05-13", k=2)
